...
const MovieSliderLayout: FC<{
  title: string;
  movieArray: any[];
  link: string;
}> = ({ title, movieArray, link }) => {
  ...
  return (
    <section className="p-4 relative flex flex-col w-full gap-4">
      ...
      <MdKeyboardArrowRight
        onClick={() => setTranslate("200px")}
        size={32}
        className="absolute top-[50%] right-4"
      />
      <div
        className={` w-[92vw] movie-slider overflow-x-scroll flex gap-10 translate-x-[${translate}]`}
      >
        {movieArray &&
