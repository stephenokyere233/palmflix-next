...
const LoginModal = () => {
    ...
    const handleGoogleAuth = () => {
        signInWithPopup(firebaseAuth, googleProvider).then(async (result: UserCredential) => {
        ...
        <ModalLayout onHideModal={() => setShowLoginModal(false)}>
            ...
            <section className=' p-2 w-full flex flex-col gap-4 justify-center items-center min-w-[350px] '>
                ...
                <div className='flex pb-2 w-full flex-col'>
                    ...
                    <input name="password" type="password" value={password} className='outline-none p-2 rounded-md bg-transparent border' onChange={(event) => {
                ...
                <button onClick={handleEmailAndPasswordAuth} className='border w-full justify-center text-center rounded-md bg-[#2221] p-2 flex items-center gap-2'>{loading ? "Loading...." : "Sign in"}</button>
                <div className='flex items-center gap-2 '>
                    <hr className='w-[100px]' />
                    <p>OR</p>
                    <hr className='w-[100px]' />
                </div>
                <button onClick={handleGoogleAuth} className='border w-full justify-center  rounded-md bg-[#2221] p-2 flex items-center gap-2'><FcGoogle /> Sign in with gmail</button>
